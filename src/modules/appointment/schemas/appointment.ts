...
export const publicBookingInputSchema = z.object({ clinicPublicSlug: z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).max(160), startsAt: z.coerce.date(), firstName: publicText, paternalLastName: publicText.optional(), maternalLastName: publicText.optional(), phone: z.string().trim().max(40).optional(), email: z.string().trim().email().max(254).optional() }).superRefine((value, context) => { if (!value.phone?.replace(/[^0-9]/g, '') && !value.email) context.addIssue({ code: z.ZodIssueCode.custom, message: 'Se requiere teléfono o correo.' }) })
export const publicNewBookingInputSchema = publicBookingInputSchema.required({ paternalLastName: true, maternalLastName: true })
...
const publicNewStudyVariant = <T extends string, S extends z.ZodTypeAny>(type: T, details: S) => publicNewBookingInputSchema.extend({ type: z.literal(type), details, doctor: doctorClientSchema.optional() })
...
export const publicNewStudyBookingInputSchema = z.union([
  publicNewStudyVariant('radiography', radiographyDetailsSchema), publicNewStudyVariant('radiography_2d', radiography2dDetailsSchema), publicNewStudyVariant('cbct', cbctDetailsSchema), publicNewStudyVariant('cephalometric_analysis', cephalometricAnalysisDetailsSchema), publicNewStudyVariant('study_models', studyModelsDetailsSchema), publicNewStudyVariant('intraoral_scan', intraoralScanDetailsSchema), publicNewStudyVariant('orthodontic_package', orthodonticPackageDetailsSchema), publicNewStudyVariant('aligner_package', alignerPackageDetailsSchema), publicNewStudyVariant('laboratory_order', laboratoryOrderDetailsSchema),
])
