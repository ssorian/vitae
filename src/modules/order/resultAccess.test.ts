...
import { allowResultAccessAttempt, grantCookieValue, hasUsableResultGrant, parseGrantCookie, resetResultAccessAttemptsForTest, resultGrantExpiresAt, secureSecretMatches, selectResultDeliveryTarget } from './resultAccess'
...
test('result secrets are hashed, expire, and use generic cookie grants', () => {
  ...
  assert.equal(resultGrantExpiresAt(now).toISOString(), '2026-08-14T10:00:00.000Z')
