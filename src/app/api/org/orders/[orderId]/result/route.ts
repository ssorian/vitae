...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (typeof observations !== 'string' || typeof realizedAt !== 'string' || Number.isNaN(new Date(realizedAt).getTime())) {
    ...
    const status = message === 'UNAUTHORIZED' || message === 'ORGANIZATION_REQUIRED' ? 401 : 400
