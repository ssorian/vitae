...
import { createAppointmentAction, listAgendaAction, listPatientsAction } from '#/modules/appointment/server/appointment'
...
export default function AppointmentsPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError('')
    try {
      await createAppointmentAction({ clinicId: formClinicId, patientId: new FormData(form).get('patientId'), startsAt, endsAt, notes: new FormData(form).get('notes') || undefined, status: new FormData(form).get('status') })
      await load()
      form.reset()
      setFormClinicId('')
      setStartAt('')
