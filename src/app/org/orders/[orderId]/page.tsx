...
import { listResourcesAction, scheduleOrderAppointmentAction } from '#/modules/appointment/server/appointment'
...
const EVENT_LABELS: Record<string, string> = {
  ...
  'order.scheduled': 'Estudio agendado',
  'order.started': 'Estudio iniciado',
  'result.uploaded': 'Archivo de resultado adjuntado',
  'result.finalized': 'Resultado finalizado',
  'email.sent': 'Resultados enviados por correo',
  'order.delivered': 'Resultados entregados',
...
export default function OrgOrderDetailPage() {
  ...
  const [resources, setResources] = useState<Awaited<ReturnType<typeof listResourcesAction>>>([])
  ...
  const loadOrder = useCallback(async () => {
    try {
      ...
      setOrder(data ?? null)
      if (data) setResources(await listResourcesAction(data.clinicId))
    } finally {
      setLoading(false)
    }
  }, [orderId])
  ...
  async function scheduleStudy() {
    if (!order || !slot || !resourceIds.length) return
    setSubmitting(true)
    try {
      const startsAt = new Date(slot)
      await scheduleOrderAppointmentAction({ orderId: order.id, startsAt, endsAt: new Date(startsAt.getTime() + 30 * 60000), resourceIds })
      await loadOrder()
    } catch { setError('No se pudo agendar el estudio.') } finally { setSubmitting(false) }
  }

  async function changeStatus(status: 'cancelled') {
    if (!order) return
    setSubmitting(true)
    try { await updateOrderStatusAction(order.id, status); await loadOrder() } catch { setError('No se pudo actualizar la orden.') } finally { setSubmitting(false) }
  }
  ...
  async function uploadResult(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'No se pudieron adjuntar los resultados.')
  ...
  async function deliver() {
    if (!order) return
    setSubmitting(true); setError('')
    try { await deliverOrderResultsAction(order.id); await loadOrder() } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudieron enviar los resultados.') } finally { setSubmitting(false) }
  }

  if (loading) return <div className="py-20 text-center text-zinc-400">Cargando orden...</div>
  if (!order) return <div className="py-20 text-center"><p>Orden no encontrada.</p><Button variant="ghost" onClick={() => router.push('/org/orders')}>Volver a órdenes</Button></div>
  ...
  return (
    <div className="mx-auto flex w-full flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3"><Button variant="ghost" size="sm" onClick={() => router.push('/org/orders')}><ArrowLeft className="mr-1 h-4 w-4" />Órdenes</Button><div><p className="font-mono font-bold text-primary">{order.folio}</p><Badge>{STATUS_LABELS[order.status] ?? order.status}</Badge></div></div>
        <div className="flex flex-wrap gap-2">
          {order.status === 'received' && <><input className="rounded border p-2 text-xs" type="datetime-local" value={slot} onChange={(event) => setSlot(event.target.value)} /><select multiple className="rounded border p-2 text-xs" value={resourceIds} onChange={(event) => setResourceIds(Array.from(event.target.selectedOptions, (item) => item.value))}>{resources.filter((item) => item.status === 'active').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><Button size="sm" disabled={submitting || !slot || !resourceIds.length} onClick={scheduleStudy}>Agendar estudio</Button></>}
          {['received', 'scheduled', 'in_progress'].includes(order.status) && <Button variant="destructive" size="sm" disabled={submitting} onClick={() => changeStatus('cancelled')}>Cancelar</Button>}
          {order.status === 'ready' && <Button size="sm" disabled={submitting} onClick={deliver}><Send className="mr-2 h-4 w-4" />{submitting ? 'Enviando...' : 'Enviar resultados'}</Button>}
        </div>
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          {canAttachResult && <Card><CardHeader><CardTitle>Finalizar y adjuntar resultados</CardTitle></CardHeader><CardContent><form className="space-y-4" onSubmit={uploadResult}><textarea required name="observations" className="min-h-24 w-full rounded border p-2 text-sm" placeholder="Observaciones" /><label className="block text-sm">Fecha realizada<input required name="realizedAt" type="date" className="ml-2 rounded border p-2" /></label><label className="block text-sm">Archivos JPG o DICOM (máximo 10; 100 MB por archivo; 500 MB total)<input required name="files" type="file" accept="image/jpeg,.jpg,.jpeg,.dcm,.dicom,application/dicom" multiple className="mt-1 block" /></label><Button disabled={submitting}>{submitting ? 'Guardando...' : 'Finalizar resultado'}</Button></form></CardContent></Card>}
          {result && <Card><CardHeader><CardTitle>Resultado finalizado</CardTitle></CardHeader><CardContent><p className="whitespace-pre-wrap text-sm">{result.observations || 'Sin observaciones.'}</p><p className="mt-2 text-xs text-zinc-500">Realizado: {result.realizedAt ? new Date(result.realizedAt).toLocaleDateString('es-MX') : '—'}</p>{hasViewerAssets && <Button className="mt-3" size="sm" onClick={() => router.push(`/org/orders/${order.id}/viewer`)}>Abrir visor</Button>}</CardContent></Card>}
        ...
        <Card><CardHeader><CardTitle>Actividad</CardTitle></CardHeader><CardContent><ol className="space-y-3">{[...(order.events ?? [])].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((event) => <li key={event.id} className="text-sm"><p>{EVENT_LABELS[event.type] ?? event.type}</p><p className="text-xs text-zinc-500">{new Date(event.createdAt).toLocaleString('es-MX')}</p></li>)}</ol></CardContent></Card>
      </div>
    </div>
  )
}
