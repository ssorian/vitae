...
export default async function PatientPage({ params }: PatientPageProps) {
  ...
  return <main className="mx-auto max-w-5xl space-y-6 p-6">
    ...
    <Card><CardHeader><CardTitle>Datos personales</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm sm:grid-cols-2"><p><span className="font-medium">Fecha de nacimiento:</span> {profile.identity.birthDate ? dateFormat.format(new Date(profile.identity.birthDate)) : 'Sin registrar'}</p><p><span className="font-medium">Teléfono:</span> {profile.identity.phone ?? 'Sin registrar'}</p><p><span className="font-medium">Correo:</span> {profile.identity.email ?? 'Sin registrar'}</p></CardContent></Card>
    ...
    <section className="space-y-3"><h2 className="text-xl font-semibold">Historial de órdenes</h2>{profile.orders.length ? <div className="grid gap-3">{profile.orders.map((item) => <Card key={item.id} size="sm"><CardContent className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">{studyTypeLabels[item.type] ?? item.type} · {item.clinicName}</p><p className="text-sm text-muted-foreground">Folio {item.folio} · {dateFormat.format(item.createdAt)}</p></div><div className="flex items-center gap-3"><Badge variant={item.status === 'ready' || item.status === 'delivered' ? 'secondary' : 'outline'}>{resultStatusLabel(item.status, Boolean(item.resultFinalizedAt))}</Badge><Link className="text-sm font-medium text-primary underline-offset-4 hover:underline" href={`/patient/${patientId}/studies/${item.id}`}>Ver estudio</Link></div></CardContent></Card>)}</div> : <p className="text-sm text-muted-foreground">Todavía no hay órdenes en tu historial.</p>}</section>
    <section className="space-y-3"><h2 className="text-xl font-semibold">Historial de tratamientos</h2><p className="text-sm text-muted-foreground">Todavía no hay tratamientos registrados.</p></section>
  </main>
}
